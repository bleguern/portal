...
<html>
<head>
<title>ADMINISTRATION - Update an account</title>
		...
		if(Session["sql_connection_string"] != null)
		{
			...
			if(!IsPostBack)
			{
				...
				if(!accountId.Equals(""))
				{
					...
					string [] values = sqlClient.GetValuesWithStoredProcedure("admin_get_account",
						"@id_account", accountId);
					
					TrigIdTextField.Value = values[0].Substring(0, 3);
					UseIdTextField.Value = values[0].Substring(7, 3);
					SetHmltSelectSelectedIndexByText(ref YearIdSelect, values[0].Substring(3, 2));
					SetHmltSelectSelectedIndexByText(ref MonthIdSelect, values[0].Substring(5, 2));
					NumberIdTextField.Value = values[0].Substring(10, 3);
					SetHmltSelectSelectedIndexByString(ref ActiveSelect, values[1]);
					SetHmltSelectSelectedIndexByString(ref UseSelect, values[2]);
					SetHmltSelectSelectedIndexByString(ref SectorSelect, values[3]);
					NameTextField.Value = values[4];
					ContractHiddenField.Value = values[5];
					RdCheckBox.Checked = Convert.ToBoolean(values[6]);
					CirCheckBox.Checked = Convert.ToBoolean(values[7]);
					BillableCheckBox.Checked = Convert.ToBoolean(values[8]);
					UseCheckBox.Checked = Convert.ToBoolean(values[9]);
					AllowanceCheckBox.Checked = Convert.ToBoolean(values[10]);
					SetHmltSelectSelectedIndexByString(ref CustomerSelect, values[11]);
					CommentTextArea.Value = values[12];
					ExtraTextField.Value = values[13];
					NoAccountableCheckBox.Checked = Convert.ToBoolean(values[14]);
				}
			}
			...
			if(!accountId.Equals(""))
			{
				if(Session["sql_connection_string"] != null)
				{
					try
					{
						...
						string result = sqlClient.ExecuteStoredProcedure("admin_update_account",
							"@id_account", accountId,
					...
					{
						Response.Redirect("../../error/Default.aspx?error=Internal error while updating an account!&message=" + ex.Message);
					}
...
<table width="500" border="0" cellspacing="0" cellpadding="0">
  <tr>
  	<td>
		<table width="500" border="0" cellspacing="0" cellpadding="0">
			<tr>
    			<td class="header">UPDATE AN ACCOUNT </td>
  			</tr>
		</table>
	</td>
   </tr>
  <tr>
    ...
    <tr>
	  <td>
		<table width="500" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td width="80" class="text"><span class="text_bold">ID</span> : </td>
			    <td width="5"></td>
			    <td width="190">
