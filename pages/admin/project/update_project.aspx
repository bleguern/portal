...
<html>
<head>
<title>ADMINISTRATION - Update a project</title>
		...
		if(projectId != "")
		{
			if(!IsPostBack)
			{
				if(Session["sql_connection_string"] != null)
				{
					...
					string [] values = sqlClient.GetValuesWithStoredProcedure("admin_get_project",
						"@id_projet_client", projectId);
				
					NameTextField.Value = values[0];
					SetHmltSelectSelectedIndexByString(ref CustomerSelect, values[1]);
					SetHmltSelectSelectedIndexByString(ref ProjectLeaderSelect, values[2]);
					SetHmltSelectSelectedIndexByString(ref IntegratorSelect, values[3]);
				}
			}
		}
	}
			...
			if(!projectId.Equals(""))
			{
				if(Session["sql_connection_string"] != null)
				{
					try
					{
						...
						string result = sqlClient.ExecuteStoredProcedure("admin_update_project",
							"@id_projet_client", projectId,
					...
					{
						Response.Redirect("../../error/Default.aspx?error=Internal error while updating a project!&message=" + ex.Message);
					}
				}
			}
		}
...
<table width="500" border="0" cellspacing="0" cellpadding="0">
  <tr>
  	<td>
		<table width="500" border="0" cellspacing="0" cellpadding="0">
			<tr>
    			<td class="header">UPDATE A PROJECT</td>
  			</tr>
		</table>
	</td>
  </tr>
