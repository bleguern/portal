...
<script language="C#" runat="server">
	void Page_Load(Object sender, EventArgs e) 
	{
		...
		if(Session["sql_connection_string"] != null)
		{
			...
			sqlClient.UpdateDataSetWithStoredProcedure(ref pageData, "donezat_time_report_page_data",
			...
			if(!IsPostBack)
			{
				...
				{
					beginDate = sqlClient.ExecuteStoredProcedure("donezat_get_last_time_report_date",
						"@login", User.Identity.Name.ToUpper());
				...
				status = sqlClient.ExecuteStoredProcedure("donezat_get_time_report_status",
					"@login", User.Identity.Name.ToUpper(),
	...
	void NextWeekButton_Click(Object sender, EventArgs e) 
	{
		...
		Response.Redirect("main.aspx?date=" + next.ToShortDateString());
	}
	...
	void PreviousWeekButton_Click(Object sender, EventArgs e) 
	{
		...
		Response.Redirect("main.aspx?date=" + previous.ToShortDateString());
	}
	...
	void GoToButton_Click(Object sender, EventArgs e) 
	{
		try
		{
			...
			Response.Redirect("main.aspx?date=" + goTo.ToShortDateString());
		}
		catch(FormatException) {}
		catch(OverflowException) {}
	}
	...
	void SaveButton_Click(Object sender, EventArgs e) 
	{
		if(Session["sql_connection_string"] != null)
		{
			...
			{
				string result = sqlClient.ExecuteStoredProcedure("donezat_delete_week_activity",
					"@login", User.Identity.Name.ToUpper(),
					"@begin_date", beginDate);
			...
			foreach(DataRow row in currentTimeReport.Rows)
			{
				...
				{
					string newId = sqlClient.ExecuteStoredProcedure("donezat_add_week_activity",
						"@login", User.Identity.Name.ToUpper(),
	...
	void ValidateButton_Click(Object sender, EventArgs e) 
	{
		if(Session["sql_connection_string"] != null)
		{
			...
			if(status.Equals("SAVED") || status.Equals("VALIDABLE") || status.Equals("VALIDATED"))
			{
				result = sqlClient.ExecuteStoredProcedure("donezat_delete_week_activity",
					"@login", User.Identity.Name.ToUpper(),
					"@begin_date", beginDate);
			...
			if(success)
			{
				result = sqlClient.ExecuteStoredProcedure("donezat_validate_time_report",
					"@login", User.Identity.Name.ToUpper(),
					"@begin_date", beginDate);
	...
	void UpdateAccountListDataGrid()
    {
       	if(Session["sql_connection_string"] != null)
		{
			...
			sqlClient.UpdateDataGridWithStoredProcedure(ref AccountListDataGrid, "donezat_get_account_list_with_user",
				"@login", User.Identity.Name.ToUpper());
		}
    }
	...
	void LoadTimeReportDataGrid()
	{
		if(Session["sql_connection_string"] != null)
		{
			...
			sqlClient.UpdateDataSetWithStoredProcedure(ref currentTimeReportDataSet, "donezat_get_saved_time_report",
				"@login", User.Identity.Name.ToUpper(),
				"@begin_date", beginDate);
...
<table width="850" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td class="header">TIME REPORT</td>
	</tr>
	<tr>
		<td height="10"></td>
	</tr>
...
<tr>
	...
	<tr>
	 <td align="left"><input name="AddLineButton" class="text" type="submit" id="AddLineButton" value="Add a line" runat="server" onServerClick="AddLineButton_Click" alt="Add a line to time report" style="width:100px" />
      ...
      <input type="hidden" name="StatusHiddenField" id="StatusHiddenField" runat="server" value="0"></td>
	</tr>
	<tr>
	 <td height="10"></td>
	</tr>
